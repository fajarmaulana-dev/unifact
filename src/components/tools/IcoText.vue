...
onMounted(() => {
    ...
    sr.onstart = () => {
        ...
        state.value = 'Ucapkan sesuatu ... (Saat selesai, ucapkan "remember" untuk memprediksi atau ucapkan "forget" untuk mereset ucapan)'
    }
    sr.onend = () => {
...
<template>
    <section
        ...
        <input ref="inbox" :title="title" :required="required" :type="type" :id="id" :name="name"
            :placeholder="placeholder" :disabled="disabled" :value="showPreview ? state : modelValue"
            @input="onModel($event)" spellcheck="false" @keyup.enter="onEnter()"
            :class="is === 'search' ? '!pr-20 md:!pr-[calc(1.25rem+6vw)]' : ''"
        ...
        <i title="Deteksi dengan ucapan" v-if="is === 'search'" @click="ToggleMic(); inbox.focus()"
